...
## Patomove Genomic MVP Decisions (December 26, 2025)

- **Isolate as the atomic entity**: each sample is modeled once, with links to assemblies, metadata, and analytics outputs.
- **Amino-acid catalog instead of nucleotide ORFs**: deduplicate on protein sequence so synonymous SNPs collapse into the same entity and we can align with global standards. Internally we key each AA sequence by a SHA256 hash; externally we reuse UniProt IDs.
- **External annotations first**: UniProt (or RefSeq/UniRef) remain the “single source of truth.” Local annotations are additive.
- **Fast ORF calling with PGAP compatibility**: Prodigal (or similar) handles draft isolates, while PGAP calls are ingested when available.

### Minimal Database Schema (AA-centric MVP)

Only critical records are stored initially; everything else can be added later.

| Table | Purpose | Key Fields |
| --- | --- | --- |
| `isolate` | master record per clinical/sample isolate | `isolate_id` (UUID), `label`, `collection_date`, `source_lab`, `notes`, timestamps |
| `protein_ref` | canonical UniProt entries tracked | `uniprot_id` (PK), `name`, `function`, `sequence_hash` (SHA256 AA) |
| `isolate_protein` | presence/absence evidence | `isolate_id`, `uniprot_id`, `evidence` (exact/partial/predicted), `pipeline_version`, `detected_at` |

**Workflow**
1. Assemble genome and call ORFs (Prodigal or PGAP).
2. Translate ORFs to amino acids and align to UniProt.
3. For each confident match, insert (`isolate_id`, `uniprot_id`) into `isolate_protein`.
4. Store the AA hash to confirm the ORF still matches UniProt when annotations update.
